...
function ReservationCard({ booking, onDelete }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row border border-primary-800">
      {/* IMAGE */}
      <div className="relative h-40 lg:h-32 lg:aspect-square w-full lg:w-auto">
        <Image
          src={image}
          alt={`Cabin ${name}`}
          fill
          className="object-cover lg:border-r border-primary-800"
        />
      </div>

      {/* CONTENT */}
      <div className="flex-grow px-4 lg:px-6 py-3 flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 mt-auto items-center">
          ...
          <p className="lg:ml-auto text-xs lg:text-sm text-primary-400">
      ...
      {!isPast(startDate) && (
        <div className=" grid grid-cols-2 lg:flex lg:flex-col border-t lg:border-t-0 lg:border-l border-primary-800 w-full lg:w-[100px]">
          <Link
            href={`/account/reservations/edit/${id}`}
            className="group flex flex-1 items-center justify-center gap-2 border-r  uppercase text-xs font-bold text-primary-300 lg:border-b lg:border-r-0 border-primary-800 px-3 py-3 hover:bg-accent-600 transition-colors hover:text-primary-900"
          >
          ...
          <DeleteReservation
            className="justify-center lg:justify-baseline"
            bookingId={id}
            onDelete={onDelete}
          />
        </div>
